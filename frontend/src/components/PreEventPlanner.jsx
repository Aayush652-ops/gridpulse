...
export default function PreEventPlanner({ t, activeLang, is3D }) {
  ...
  const runSimulation = async (event, step) => {
    ...
    setLoadingSim(true);
    try {
      const url = getApiUrl(`/api/forecast-propagation?latitude=${event.latitude}&longitude=${event.longitude}&event_cause=${event.event_cause}&priority=${event.priority}&time_step_hours=${step}&duration_hours=${event.duration_hours}&attendance=${event.attendance || 1000}`);
  ...
  const handleDelete = async (eventId, e) => {
    ...
    try {
      const res = await fetch(getApiUrl(`/api/planned-events/${eventId}`), {
        method: 'DELETE'
      });
      if (res.ok) {
        await fetchEvents();
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      for (let hour = 0; hour <= totalHours; hour++) {
        ...
        const url = getApiUrl(`/api/forecast-propagation?latitude=${selectedEvent.latitude}&longitude=${selectedEvent.longitude}&event_cause=${selectedEvent.event_cause}&priority=${selectedEvent.priority}&time_step_hours=${hour}&duration_hours=${selectedEvent.duration_hours}&attendance=${selectedEvent.attendance || 1000}`);
        const res = await fetch(url);
